...
[npm-image]: https://img.shields.io/npm/v/@sol-fx-io/sol-fx-sdk.svg?style=flat
[npm-url]: https://www.npmjs.com/package/@sol-fx-io/sol-fx-sdk

[![npm][npm-image]][npm-url]

An SDK for building applications on top of Sol-FX.
...
```bash
$ yarn add @sol-fx-io/sol-fx-sdk
```

### npm

```bash
$ npm install @sol-fx-io/sol-fx-sdk --save
```
...
- Solana format: https://api.sol-fx.io/v2/sdk/token/solana.mainnet.json
- Sol-FX format: https://api.sol-fx.io/v2/sdk/token/sol-fx.mainnet.json
...
- https://api.sol-fx.io/v2/sdk/liquidity/mainnet.json
...
// sol-fx pool id can get from api: https://api.sol-fx.io/v2/sdk/liquidity/mainnet.json
...
export async function parsePoolInfo() {
  ...
  const openOrders = await OpenOrders.load(
    ...
    OPENBOOK_PROGRAM_ID // OPENBOOK_PROGRAM_ID(marketProgramId) of each pool can get from api: https://api.sol-fx.io/v2/sdk/liquidity/mainnet.json
  );
...
- https://api.sol-fx.io/v2/sdk/farm/mainnet.json

#### Example of parse farm info

```typescript
import { Connection, PublicKey } from "@solana/web3.js";
import { Farm } from "@sol-fx-io/sol-fx-sdk";
import {
  JsonPairItemInfo,
  FarmPoolsJsonFile,
  FarmPoolJsonInfo,
...
// sol-fx farm id can get from api: https://api.sol-fx.io/v2/sdk/farm-v2/mainnet.json
...
export async function demoFarm() {
  ...
  const { data: farmData } = await axios.get<FarmPoolsJsonFile>(
    "https://api.sol-fx.io/v2/sdk/farm-v2/mainnet.json"
  );

  console.log("fetching pairs");
  const { data: pairData } = await axios.get<JsonPairItemInfo[]>(
    "https://api.sol-fx.io/v2/main/pairs"
  );
  ...
  }>("https://api.sol-fx.io/v2/sdk/token/sol-fx.mainnet.json");
  ...
  const { data: tokenPrices } = await axios.get<{ [key: string]: number }>(
    "https://api.sol-fx.io/v2/main/price"
  );
...
export interface FarmPoolJsonInfo {
  ...
  category: "stake" | "sol-fx" | "fusion" | "ecosystem"; // add by UI for unify the interface
}
...
export type FarmPoolsJsonFile = {
  ...
  sol-fx: Omit<FarmPoolJsonInfo, "category">[];
